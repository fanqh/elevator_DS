...
void handleUnrecognised(const uint8 *data, uint16 length, Task task) {
	...
	sppb_task_t* task_data = (sppb_task_t*)task;
	task_data ->command_result = CMD_RET_UNRECOGNIZED;
}
...
void connect(Task task, const struct connect * config) {
    ...
    task_data ->uart_keeptime = config->keeptime;
    if(config ->polarity==1)
    {
        
        task_data ->uart_polarity = 1;
	    /** set output and drive high 
	    PioSetDir(PIO3, PIO3);
	    PioSet(PIO3, PIO3); 
        MessageSendLater(task, SPP_ECHO_PIOSTATE_TIMEOUT, 0, keeptime);**/
    }
    else if(config ->polarity==0)
    {
        task_data ->uart_polarity = 0;
        /** set output and pull down 
	    PioSetDir(PIO3, PIO3);
	    PioSet(PIO3, 0);
        MessageSendLater(task, SPP_ECHO_PIOSTATE_TIMEOUT, 0, keeptime);**/
    }
    else if(config ->polarity==2)
    {
        task_data ->uart_polarity = 2;
        /*
        PioSetDir(PIO3, PIO3);
    ...
    {
        task_data ->command_result = CMD_RET_UNSUPPORTED_P0LARITY;
		return;
    }
